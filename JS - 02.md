...
+ **`document.querySelectorAll(selector)`**
  ...
  + 지정된 selector에 일치하는 **NodeList 반환**
...
`liTag.classList.toggle('red-text')` : 클릭 할 때마다 상태값을 변경(생성,삭제 번갈아가며 )시키고 싶은 경우
...
+ 근데, 추가인지, 저걸로만 설정하는 것인지 확인해봐야함
  + 아마 가장 마지막 setAttribute의 것만 추가되는 듯
